...
// Saldo atual da conta = saldo_inicial + fundos (aportes - saques) + receitas - despesas
export const calcSaldoConta = (conta, movimentacoes, fundos = []) => {
  ...
  const fromFundos = fundos
    .filter((f) => f.conta_nome === conta.nome)
    .reduce((a, f) => (f.tipo === 'aporte' ? a + Number(f.valor) : f.tipo === 'saque' ? a - Number(f.valor) : a + Number(f.valor)), 0)
  return base + fromMovs + fromFundos
}

export const calcPatrimonio = (contas, movimentacoes, fundos = []) =>
  contas.reduce((total, c) => total + calcSaldoConta(c, movimentacoes, fundos), 0)
